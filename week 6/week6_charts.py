...
def chart2_2025_criminal_vs_other(df: pd.DataFrame) -> go.Figure:
    """Trial Q4: criminal-style vs other captions in 2025 (horizontal bar)."""
    ...
    fig = go.Figure(
        data=[
            go.Bar(
                ...
                x=counts,
                orientation="h",
                marker_color=["#E45756", "#72B7B2"],
                text=[f"{n:,}" for n in counts],
                textposition="outside",
                hovertemplate="%{y}<br>%{x:,} opinion rows<extra></extra>",
            )
        ]
    )
    fig.update_layout(
...
def combined_interactive_html(df: pd.DataFrame | None = None) -> go.Figure:
    ...
    fig.update_xaxes(title_text="Opinion rows", tickformat=",", row=2, col=1)
    fig.update_xaxes(title_text="Opinion rows", tickformat=",", row=3, col=1)
    return _apply_interactive(fig)
